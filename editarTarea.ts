...
export function editarTarea(tarea: Tarea): void {
  ...
  let nuevoTitulo: string = prompt("Nuevo título (dejar en blanco para mantener el actual):");
  if (nuevoTitulo) tarea['titulo'] = nuevoTitulo;

  let nuevaDescripcion: string = prompt("Nueva descripción (dejar en blanco para mantener el actual):");
  if (nuevaDescripcion) tarea['descripcion'] = nuevaDescripcion;

  let nuevoEstado: string = prompt("Nuevo estado (Pendiente, En curso, Terminada, Cancelada):");
  ...
  if (nuevaDificultad) tarea['dificultad'] = parseInt(nuevaDificultad);
